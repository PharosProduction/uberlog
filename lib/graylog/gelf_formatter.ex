defmodule Uberlog.Graylog.GelfFormatter do
  @moduledoc false
  ...
  @spec format(host, Logger.level(), Logger.message(), :calendar.datetime(), Logger.metadata()) :: iodata
  def format(host, level, message, timestamp, metadata), do: format(host, level, message, timestamp, metadata, [])

  @spec format(host, Logger.level(), Logger.message(), :calendar.datetime(), Logger.metadata(), Keyword.t()) :: iodata
  def format(host, level, message, timestamp, metadata, opts) do
    ...
    |> Enum.map(fn {k, v} -> {["_", to_string(k)], v} end) 
    |> Enum.into(%{})
  ...
  defp maybe_timestamp_field(timestamp, opts) do
    case Keyword.get(opts, :include_timestamp, true) do
      true -> %{"timestamp" => format_timestamp(timestamp)}
      false -> %{}
    end
  end
