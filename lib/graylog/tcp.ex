defmodule Uberlog.Graylog.Tcp do
  @moduledoc false

  require Logger

  alias Uberlog.Graylog.GelfFormatter, as: Formatter
  ...
  @impl :gen_event
  def init(__MODULE__), do: init(app_env_config())
  ...
  def handle_call({:configure, opts}, state) do
    case configure(state, opts) do
      ...
      {:error, _} = err -> {:ok, err, state}
    end
  end

  @impl :gen_event
  def handle_event({level, gl, {Logger, message, timestamp, metadata}}, %{socket: {:connected, _}} = state) when node(gl) == node() do
    with true <- should_log?(level, state),
    :ok <- log(level, message, timestamp, metadata, state) do
      {:ok, state}
    else
      :error -> {:ok, try_connect(%{state | socket: :disconnected})}
      _ -> {:ok, state}
    end
  end

  def handle_event(_, %{socket: :disconnected} = state), do: {:ok, state}

  def handle_event(_, state), do: {:ok, state}

  @impl :gen_event
  def handle_info({:tcp_closed, socket}, %{socket: {:connected, socket}, host: host, port: port} = state) do
    Logger.error(fn -> "Connection with #{format_endpoint(host, port)} closed" end)
    {:ok, try_connect(%{state | socket: :disconnected})}
  end

  def handle_info({:timeout, _, :reconnect}, %{socket: :disconnected} = state), do: {:ok, try_connect(state)}
  ...
  @spec log(Logger.level(), Logger.message(), Formatter.timestamp(), Logger.metadata(), state) :: :ok | :error
  defp log(level, message, timestamp, metadata, state) do
    %{socket: {:connected, socket}, host: host, port: port, include_timestamp: include_timestamp} = state
    metadata_to_send = extract_metadata(level, message, timestamp, metadata, state.metadata_filter)
    log = Formatter.format(state.gelf_host, level, message, timestamp, metadata_to_send, include_timestamp: include_timestamp)

    case :gen_tcp.send(socket, [log, 0]) do
      :ok -> :ok
      {:error, reason} ->
        Logger.error(fn ->"Couldn't send log message to Graylog at #{format_endpoint(host, port)}: #{inspect(reason)}" end)
        :error
    end
  end

  @spec extract_metadata(Logger.level(), Logger.message(), Formatter.timestamp(), Logger.metadata(), metadata_filter) :: Logger.metadata()
  defp extract_metadata(_, _, _, metadata, :all), do: metadata

  defp extract_metadata(level, message, ts, metadata, {module, function}), do: apply(module, function, [level, message, ts, metadata])
  ...
  defp get_gelf_host(host) when is_binary(host), do: host
  ...
  defp try_connect(%{socket: :disconnected, host: host, port: port, backoff: backoff} = state) do
    case :gen_tcp.connect(host, port, [:binary, active: false], 5000) do
      ...
      {:error, reason} ->
        ...
        Logger.error(fn -> "Couldn't connect to #{format_endpoint(host, port)}: #{inspect(reason)}. Retrying in #{reconnect_in}s"
        end)
  ...
  defp format_endpoint({_, _, _, _} = ipv4, port), do: format_endpoint(:inet.ntoa(ipv4), port)

  defp format_endpoint({_, _, _, _, _, _, _, _} = ipv6, port), do: format_endpoint(:inet.ntoa(ipv6), port)

  defp format_endpoint(host, port), do: "#{host}:#{port}"
  ...
  defp maybe_host_to_charlist(%{host: host} = map) when is_binary(host), do: %{map | host: to_charlist(host)}

  defp maybe_host_to_charlist(map), do: map

  @spec app_env_config() :: Keyword.t()
  defp app_env_config(), do: Application.get_env(:logger, __MODULE__, [])

  @spec initial_opts_schema() :: Optium.schema()
  defp initial_opts_schema, do: %{
  ...
  defp reconfigure_opts_schema(), do: %{
  ...
  defp configure(state, config) do
    case Optium.parse(config, reconfigure_opts_schema()) do
      {:ok, opts} ->
        new_gelf_host = case Keyword.has_key?(opts, :override_host) do
          true -> get_gelf_host(opts[:override_host])
          false -> state.gelf_host
        end

        new_include_timestamp = case is_boolean(opts[:include_timestamp]) do
          true -> opts[:include_timestamp]
          false -> state.include_timestamp
        end
